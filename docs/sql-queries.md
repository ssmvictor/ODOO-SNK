...
```sql
WITH ... -- CTEs PROD_BASE, PEM, HIST, TOP_SALES, TOP_PURCHASES, TOP_POS
SELECT
  PRO.CODPROD,
  PRO.DESCRPROD,
  PRO.REFFORN,
...
**Campos retornados principais:** `CODPROD`, `DESCRPROD`, `REFFORN`, `PESOBRUTO`, `CODVOL`, `USOPROD`, `USOPROD_FISCAL`, `NCM`, `UNITRIBUTACAO`, `CODESPECST`, `ORIGPROD`, `TIPOITEMSPED`, `TEMISS`, `MARCA`, `CODLOCALPADRAO`, `CODTIPOPER_SALES`, `CODTIPOPER_PURCHASES`, `CODTIPOPER_POS`, `DESCROPER_SALES`, `DESCROPER_PURCHASES`, `DESCROPER_POS`

**Personalizacoes comuns:**
- Remover o filtro `substr` para sincronizar todos os produtos
- Selecionar empresa no subselect de `TGFPEM` para respeitar regra fiscal por empresa
